...
const ProjectBoard: React.FC<{
    ...
    const latestPipelineLog = pipelineLogs[pipelineLogs.length - 1];
    const recentPipelineLogs = pipelineLogs.slice(-12).reverse();
                    ...
                    <div className="absolute inset-0 z-50 bg-white/80 backdrop-blur-md flex flex-col items-center justify-center p-8 text-center">
                        <div className="max-w-md w-full space-y-6">
                            ...
                            <button
                                ...
                                className="px-6 py-2 border-2 border-pink-500 text-pink-500 font-bold rounded-full hover:bg-pink-50 transition-colors pointer-events-auto mt-2"
                            >
                                Cancel Production
                            </button>
                            {latestPipelineLog && (
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="bg-white border border-slate-200 p-4 rounded-2xl shadow-sm space-y-3">
                            <div className="flex items-center justify-between">
                                <h4 className="text-sm font-bold uppercase tracking-wider text-slate-700">Pipeline Diagnostics</h4>
                                <span className="text-[10px] font-mono text-slate-400">{pipelineLogs.length} events</span>
                            </div>
                            {recentPipelineLogs.length === 0 ? (
                                <p className="text-xs text-slate-400">No diagnostics yet.</p>
                            ) : (
                                <div className="space-y-2 max-h-52 overflow-y-auto pr-1">
                                    {recentPipelineLogs.map((entry) => (
                                        <div
                                            key={entry.id}
                                            className={`rounded-lg border px-3 py-2 text-xs ${entry.level === 'error'
...
const AgentChat: React.FC<{
    onGenerate: (prompt: string, attachments?: ChatAttachment[]) => Promise<{ success: boolean; cancelled?: boolean; issueCount?: number }>;
    ...
    const handleSend = async () => {
        ...
        if (isGenerationRequest && !project) {
            ...
            if (outcome.cancelled) {
                ...
                followup = "I ran into an issue during production. Open Director's View to see pipeline diagnostics.";
            } else if ((outcome.issueCount || 0) > 0) {
                followup = `Production finished with ${outcome.issueCount} recoverable issue(s). Open Director's View to inspect diagnostics.`;
            }
...
export const App: React.FC = () => {
    ...
    const generationControlRef = useRef<{ cancelled: boolean } | null>(null);
    ...
    useEffect(() => {
        if (!project) {
            localStorage.removeItem('adstudio_project');
            return;
        }
        try {
            localStorage.setItem('adstudio_project', JSON.stringify(project));
        } catch (e) {
            console.warn("localStorage quota exceeded, saving without media assets:", e);
            const strippedProject = {
                ...project,
                scenes: project.scenes.map((s: any) => ({
                    ...s,
                    storyboardUrl: s.storyboardUrl?.startsWith('data:') || s.storyboardUrl?.startsWith('blob:') ? undefined : s.storyboardUrl,
                    videoUrl: s.videoUrl?.startsWith('data:') || s.videoUrl?.startsWith('blob:') ? undefined : s.videoUrl
                })),
                voiceoverUrl: project.voiceoverUrl?.startsWith('data:') || project.voiceoverUrl?.startsWith('blob:') ? undefined : project.voiceoverUrl,
                musicUrl: project.musicUrl?.startsWith('data:') || project.musicUrl?.startsWith('blob:') ? undefined : project.musicUrl,
            };
            try {
                localStorage.setItem('adstudio_project', JSON.stringify(strippedProject));
            } catch (e2) {
                console.error("Failed to save even stripped project to localStorage", e2);
            }
        }
    }, [project]);
    ...
    const appendPipelineLog = useCallback((entry: PipelineLogEntry) => {
        setPipelineLogs(prev => [...prev.slice(-199), entry]);
    }, []);

    const pushManualPipelineLog = useCallback((entry: Omit<PipelineLogEntry, 'id' | 'timestamp'>) => {
        appendPipelineLog({
            ...entry,
            id: `manual-log-${Date.now()}-${Math.random().toString(36).slice(2, 7)}`,
            timestamp: new Date().toISOString(),
        });
    }, [appendPipelineLog]);

    const handleCancelGeneration = useCallback(() => {
        if (generationControlRef.current) {
            generationControlRef.current.cancelled = true;
        }
        setProject(prev => prev ? { ...prev, isGenerating: false, currentPhase: prev.currentPhase === 'ready' ? 'ready' : 'planning' } : null);
    ...
    const handleGenerate = async (
    ...
    ): Promise<{ success: boolean; cancelled?: boolean; issueCount?: number }> => {
        const control = { cancelled: false };
        generationControlRef.current = control;
        ...
        try {
            const run = await runGenerationPipeline({
                ...
                settings,
                files,
                visualAnchorDataUrl: visualAnchor?.content,
                preferredVoice: settings.preferredVoice === 'auto' ? TTSVoice.Kore : settings.preferredVoice,
                shouldCancel: () => control.cancelled,
                onProjectInitialized: (nextProject) => setProject(nextProject),
                onProjectUpdate: (nextProject) => setProject(nextProject),
                onLog: appendPipelineLog,
            });
            ...
            if (run.issues.length > 0) {
                pushManualPipelineLog({
                    stage: 'ready',
                    level: 'warn',
                    message: `Pipeline completed with ${run.issues.length} recoverable issue(s).`,
                });
            }
                    ...
                    if (dbId) {
                        ...
                        if (run.musicUrl && (run.musicUrl.startsWith('blob:') || run.musicUrl.startsWith('data:'))) {
                            ...
                            pushManualPipelineLog({
                                stage: 'scoring',
                                level: 'warn',
                                message: 'Music output is a remote URL and was skipped for backend asset upload.',
                            });
                        }
                        pushManualPipelineLog({
                            stage: 'ready',
                            level: 'info',
                            message: 'All generated assets persisted to backend storage.',
                        });
                    }
                } catch (persistErr) {
                    console.warn('⚠️ Backend persistence failed (non-critical):', persistErr);
                    pushManualPipelineLog({
                        stage: 'ready',
                        level: 'warn',
                        message: `Backend persistence failed: ${persistErr instanceof Error ? persistErr.message : String(persistErr)}`,
                    });
                }
            }
            return { success: true, issueCount: run.issues.length };
        } catch (e) {
            ...
            generationControlRef.current = null;
            setProject(prev => prev ? { ...prev, isGenerating: false } : null);
        }
    };
                ...
                <div className="flex items-center gap-4">
                    ...
                    {project && (
                        <button
                            ...
                            className="hidden md:block bg-white/50 hover:bg-white text-slate-700 font-bold px-4 py-1.5 rounded-full text-sm transition-colors border border-slate-200"
                        >
                            + New Project
                        </button>
                    )}
